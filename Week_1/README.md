...
1. **Inspected the data** using `.info()` and `.shape` to understand structure, column types, and non-null counts.
2. **Checked for missing values** with `.isnull().sum()`. Found missing data in:
...
3. **Checked for duplicate rows** using `.duplicated()` and removed any found with `.drop_duplicates()`.
4. **Fixed incorrect data types** — converted `Survived`, `Pclass`, `Sex`, and `Embarked` from raw numbers/generic objects into proper `category` types, since they represent labels rather than continuous quantities.
5. **Verified the result** — confirmed no missing values remain and all data types are appropriate, then saved the cleaned dataset to `titanic_cleaned.csv`.
